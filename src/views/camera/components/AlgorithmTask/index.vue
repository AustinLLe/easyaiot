                ...
                <div class="task-info">
                  ...
                  <div class="btns">
                    ...
                    <div 
                      ...
                      :title="item.is_enabled ? '任务运行中，无法配置' : !hasModels(item) ? '请先配置算法模型列表' : !hasCameras(item) ? '请先配置摄像头列�? : '区域检测配�?"
                    >
                    ...
                    <div 
                      ...
                      :title="!hasCameras(item) ? '请先配置摄像头列�? : '查看抓拍空间'"
                    >
...
// 检查任务是否有摄像头列�?const hasCameras = (record: AlgorithmTask) => {
...
// 复制摄像头名�?const handleCopyDeviceNames = (item: AlgorithmTask) => {
...
const getTableActions = (record: AlgorithmTask) => {
    ...
    },
    {
      ...
      onClick: () => {
        ...
        if (!hasCameras(record)) {
          createMessage.warning('请先配置摄像头列�?);
          return;
        }
  ...
  if (record.task_type === 'snap') {
    actions.push({
      icon: 'ant-design:folder-outlined',
      tooltip: !hasCameras(record) ? '请先配置摄像头列�? : '查看抓拍空间',
...
const [registerForm, { validate }] = useForm({
  ...
  labelWidth: 80,
  baseColProps: { span: 6 },
  // 将按钮放到第一行，与第三个字段同一�?  actionColOptions: { span: 6, offset: 0, style: { textAlign: 'right' } },
...
const handleOpenRegionDetection = (record?: AlgorithmTask) => {
  // 校验：只有在停用状态下才能配置区域检�?  if (record && record.is_enabled) {
    createMessage.warning('任务运行中，无法配置，请先停止任�?);
    return;
  }
  if (record) {
    // 传入任务ID，只显示该任务关联的摄像�?    openRegionDrawer(true, { taskId: record.id });
  } else {
...
const handleStart = async (record: AlgorithmTask) => {
  try {
    ...
    // 由于 isTransformResponse: true，成功时返回的是任务对象（data.data），而不是包�?code 的响应对�?    if (response && (response as any).id) {
...
const handlePlayStream = async (record: AlgorithmTask) => {
  ...
  try {
    ...
    // 所�?response 可能是数组，也可能是包含 code 的完整响应对�?    const response = await getTaskStreams(record.id);
