	      ...
	      if ((face && (face.exact === 'user' || face.exact === 'environment' ||
	        ...
	        if (matches) {
	          ...
	          .then(devices => {
	            ...
	            if (dev) {
	              constraints.video.deviceIdentification = face.exact ? {exact: dev.deviceIdentification} :
	                                                        {ideal: dev.deviceIdentification};
	            }
	...
	class MediaStreamFactory {
	  ...
	  static createMediaStream(constraints) {
	    ...
	    if (typeof constraints.audio === 'object' && constraints.audio.source === AudioSourceInfo.MIC) {
	      ...
	      if (isEdge()) {
	        mediaConstraints.audio.deviceIdentification = constraints.audio.deviceIdentification;
	      } else {
	        mediaConstraints.audio.deviceIdentification = {
	          exact: constraints.audio.deviceIdentification
	        };
	      }
	    } else {
	    ...
	    if (typeof constraints.video === 'object') {
	      ...
	      if (typeof constraints.video.deviceIdentification === 'string') {
	        mediaConstraints.video.deviceIdentification = {
	          exact: constraints.video.deviceIdentification
	        };
	      }
